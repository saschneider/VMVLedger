#
# Quorum helper for the file contract class.
#
# @author Matthew Casey
#
# (c) University of Surrey 2019
#

class QuorumFileHelper < QuorumHelper

  # File contract constants. See vendor/solidity for contract source.
  CONTRACT_NAME      = 'ContractFile'
  CONTRACT_FILE_ABI  = [{ 'constant': true, 'inputs': [], 'name': 'getNumberOfChunks', 'outputs': [{ 'name': '', 'type': 'uint256' }], 'payable': false, 'stateMutability': 'view', 'type': 'function' }, { 'constant': true, 'inputs': [{ 'name': 'index', 'type': 'uint256' }], 'name': 'getChunk', 'outputs': [{ 'name': '', 'type': 'string' }], 'payable': false, 'stateMutability': 'view', 'type': 'function' }, { 'constant': false, 'inputs': [{ 'name': 'chunk', 'type': 'string' }], 'name': 'addChunk', 'outputs': [{ 'name': '', 'type': 'uint256' }], 'payable': false, 'stateMutability': 'nonpayable', 'type': 'function' }].freeze
  CONTRACT_FILE_CODE = ''.freeze

  CONTRACT_GAS_PRICE = 0.freeze
  CONTRACT_GAS_LIMIT = 4_000_000.freeze

  #
  # Creates a new file contract and wraps it in a wrapper object.
  #
  # @param wrapper The optional wrapper class name. Defaults to FileWrapper.
  # @returns The new file contract wrapped in an object of the class.
  #
  def create(wrapper = FileWrapper.name)
    contract = create_contract(CONTRACT_NAME, CONTRACT_FILE_ABI, CONTRACT_FILE_CODE, gas_price: CONTRACT_GAS_PRICE, gas_limit: CONTRACT_GAS_LIMIT)
    wrapper.constantize.new(contract)
  end

  #
  # Gets an existing file contract given it's address and wraps it in a wrapper object.
  #
  # @param address The address of the existing contract object.
  # @param wrapper The optional wrapper class name. Defaults to FileWrapper.
  # @returns The existing file contract wrapped in an object of the class.
  #
  def get(address, wrapper = FileWrapper.name)
    contract = get_contract(CONTRACT_NAME, CONTRACT_FILE_ABI, address, gas_price: CONTRACT_GAS_PRICE, gas_limit: CONTRACT_GAS_LIMIT)
    wrapper.constantize.new(contract)
  end

  #
  # @return An appropriate FileWrapper class name for the specified content type.
  #
  def self.wrapper_for_content(content_type)
    case content_type
      when CSVFileWrapper::CONTENT_TYPE
        CSVFileWrapper.name
      else
        BinaryFileWrapper.name # Can handle any type of binary and non-binary file.
    end
  end
end